...
if ($post_send_flag && isset($request_data["mode"]) && $request_data["mode"] == "del") {
	...
	header("Location: ../comp.html?md=temple_del");
}
elseif ($post_send_flag && isset($request_data["mode"]) && $request_data["mode"] == "image_del1") {
	...
	header("Location: temple_detail.html?" . $request_data["query"]);
}
elseif ($post_send_flag && isset($request_data["mode"]) && $request_data["mode"] == "image_del") {
	...
	header("Location: temple_list.html?" . $request_data["query"]);
}
elseif ($post_send_flag && isset($request_data["mode"]) && $request_data["mode"] == "download") {
	//CSVダウンロード
	$data = $request_data;
	$file_name = "temple_" . $lang_ary[$data["lang_code"]] . "_" . date("Ymd") . ".csv";
	$csv_data = "";
	...
	for ($i = 0; $i < count_ary($temple_header); $i++) {
		if ($i) {
			$csv_data .= ",";
		}
		$csv_data .= $temple_header[$i];
	}
	$csv_data .= "\n";
	...
	if (count_ary($list) > 0) {
		for ($i = 0; $i < count_ary($list); $i++) {
			for ($j = 0; $j < count_ary($temple_keys); $j++) {
				if ($j) {
					$csv_data .= ",";
				}
				$csv_data .= "\"" . $list[$i][$temple_keys[$j]] . "\"";
			}
			$csv_data .= "\n";
		}
	}
...
elseif ($post_send_flag && isset($request_data["mode"]) && $request_data["mode"] == "upload") {
	...
	header("Location: " . $document_path . "comp" . EXT . "?md=" . $menu_id . "_upload&menu_id=" . $menu_id);
	exit;
}
elseif ($post_send_flag && $mode == "up") {
	...
	if (!$obj_data->check_error($request_data, $dbh)) {
		...
		if ($obj_data->error_num == 1) {
			$_SESSION[$update_session_name]["temple_num"] = "";
		}
...
*/
	if (!$request_data["comp_flag"]) {
		...
		//確認画面へ
		header("Location: temple_detail.html?md=conf");
		exit;
	}
	...
	if (isset($request_data["id"]) && $request_data["id"]) {
		header("Location: ../comp.html?md=temple_chg");
	}
	else {
		header("Location: ../comp.html?md=temple_up");
	}
	exit;
}
elseif ($mode == "delete") {
