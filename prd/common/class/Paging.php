...
class Paging{
	...
	public $next_str = '次へ';
	//ulに付くclass
	public $ul_class = 'news__page-list';
	...
	public function setHtml($sum = 0){
		...
		if($this -> page != 1 && $this -> prev_str){
			$aryList[] = '<li class="news__page-item">'.$this -> getQuery(($this -> page - 1), $this -> prev_str).'</li>';
		}
		
		foreach($aryResult as $key => $value){
			if(is_numeric($value)){
				$aryList[] = '<li class="news__page-item">'.$this -> getQuery($value, $value).'</li>';
			}else{
				$aryList[] = '<li class="news__page-item">'.$this -> getQuery(0, $value).'</li>';
			}
		}
		
		if($this -> page != $max_page && $this -> next_str){
			$aryList[] = '<li class="news__page-item">'.$this -> getQuery(($this -> page + 1), $this -> next_str).'</li>';
		}
	...
	private function getQuery($page, $str){
		...
		if($this -> page == $page){
			return '<span class="is-active">'.$str.'</span>';
		}
		...
		if(!$this -> atag_remove_flg){
			...
			if($aryQuery){
				return '<a class="news__page-link" href="'.$url.'?'.http_build_query($aryQuery).'">'.$str.'</a>';
			}else{
				return '<a class="news__page-link" href="'.$url.'">'.$str.'</a>';
			}
		}else{
